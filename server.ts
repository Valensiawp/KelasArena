...
async function startServer() {
  ...
  // Safe multi-model caller with automatic fallback
  async function generateGeminiContent(prompt: string, responseMimeType = 'application/json'): Promise<string | null> {
  ...
  // Helper to extract a short, clean topic title from potentially long text/PDF content
  function extractShortTopic(rawText: string): string {
    if (!rawText) return 'Materi Pembelajaran';
    let clean = rawText.trim();
    
    // Remove common teacher labels/headings
    ...
    // Split on section markers like "I. Kemampuan...", "A. Konsep...", "Indikator:", "Tujuan:"
  ...
  // Diverse Fallback Generator for Quizzes (prevents repetitive questions)
  function createDiverseFallbackQuestions(topicText: string, questionCount: number, questionType: string, gradeLevel: string) {
    ...
    const pgTemplates = [
      {
        q: (t: string) => `Apa definisi utama dan konsep fundamental yang mendasari pembahasan mengenai ${t}?`,
        opts: (t: string) => [
        ...
        ans: (t: string) => `A. Prinsip dasar, variabel, dan teori pokok dalam ${t}`,
        exp: (t: string) => `Penguasaan ${t} dimulai dari pemahaman konsep inti dan variabel utamanya.`
      },
      {
        q: (t: string) => `Manakah contoh penerapan atau studi kasus materi ${t} yang paling tepat untuk jenjang ${gradeLevel || 'sekolah'}?`,
        opts: (t: string) => [
        ...
        ans: (t: string) => `A. Pemecahan masalah terstruktur berbasis prinsip dan rumus ${t}`,
        exp: (t: string) => `Penerapan ${t} menuntut langkah penyelesaian sistematis sesuai kaidah materi.`
      },
      {
        q: (t: string) => `Mengapa analisis dan pemahaman mendalam tentang ${t} sangat penting dalam pembelajaran siswa?`,
        opts: (t: string) => [
        ...
        ans: (t: string) => `A. Membentuk pola pikir kritis dan keterampilan bernalar logis`,
        exp: (t: string) => `Menganalisis ${t} mengasah daya nalar kritis siswa dalam memecahkan masalah.`
      },
      {
        q: (t: string) => `Ciri utama yang membedakan karakteristik konsep ${t} dibanding topik pembelajaran lainnya adalah...`,
        opts: (t: string) => [
        ...
        ans: (t: string) => `A. Adanya pola terstruktur, hukum logika, dan keterkaitan antar variabel`,
        exp: (t: string) => `Karakteristik khas ${t} terletak pada keteraturan pola dan logika penyelesaiannya.`
      },
      {
        q: (t: string) => `Langkah pertama yang paling krusial ketika menghadapi soal atau permasalahan terkait ${t} adalah...`,
        opts: (t: string) => [
        ...
        ans: (t: string) => `A. Mengidentifikasi informasi kunci, variabel, dan komponen utama ${t}`,
        exp: (t: string) => `Identifikasi elemen dan variabel kunci pada ${t} merupakan fondasi penyelesaian soal.`
      },
      {
        q: (t: string) => `Dalam melakukan evaluasi penyelesaian soal ${t}, aspek apa yang harus diperhatikan agar hasil analisis valid?`,
        opts: (t: string) => [
        ...
        ans: (t: string) => `A. Kesesuaian metode penyelesaian dengan aturan ilmiah ${t}`,
        exp: (t: string) => `Evaluasi pada ${t} memastikan setiap langkah mengikuti prosedur dan konsep yang sah.`
      },
      {
        q: (t: string) => `Bagaimana hubungan antara pemahaman teori ${t} dengan kemampuan pemecahan masalah konteks nyata?`,
        opts: (t: string) => [
        ...
        ans: (t: string) => `A. Teori ${t} menjadi landasan dasar untuk merumuskan solusi tepat`,
        exp: (t: string) => `Penguasaan konsep ${t} mempermudah penyelesaian kasus di dunia nyata.`
      },
      {
        q: (t: string) => `Strategi efektif untuk menghindari kekeliruan atau miskonsepsi saat mempelajari ${t} adalah...`,
        opts: (t: string) => [
        ...
        ans: (t: string) => `A. Berlatih secara berulang, memahami konsep dasar, dan diskusi terarah`,
        exp: (t: string) => `Diskusi dan latihan terarah dapat mengeliminasi miskonsepsi pada materi ${t}.`
      }
    ];

    const uraianTemplates = [
      {
        q: (t: string) => `Jelaskan secara mendalam pengertian, konsep dasar, serta komponen utama yang membangun materi ${t}!`,
        ans: (t: string) => `Kunci Jawaban: Penjelasan rinci mencakup definisi resmi ${t}, batasan konsep, serta komponen pembentuk utamanya.`,
        exp: (t: string) => `Skor maksimal diberikan jika siswa menjelaskan definisi dan seluruh komponen ${t} secara rinci.`
      },
      {
        q: (t: string) => `Uraikan minimal 3 prinsip pokok atau hukum yang berlaku pada topik ${t} beserta contoh kasusnya!`,
        ans: (t: string) => `Kunci Jawaban: Menyebutkan 3 prinsip utama ${t} dan memberikan contoh penerapannya yang relevan.`,
        exp: (t: string) => `Skor dihitung dari kelengkapan 3 prinsip serta keakuratan contoh kasus ${t}.`
      },
      {
        q: (t: string) => `Berikan analisis langkah-langkah sistematis dalam memecahkan permasalahan atau soal kompleks terkait ${t}!`,
        ans: (t: string) => `Kunci Jawaban: Menyajikan langkah runtut mulai dari identifikasi masalah, perumusan, hingga penarikan kesimpulan pada ${t}.`,
        exp: (t: string) => `Siswa dinilai dari keruntutan logika berpikir dan ketepatan prosedur pada ${t}.`
      },
      {
        q: (t: string) => `Analisislah miskonsepsi atau kesalahan umum yang sering terjadi saat mempelajari ${t}, dan berikan solusi mengatasinya!`,
        ans: (t: string) => `Kunci Jawaban: Menguraikan bentuk kekeliruan umum pada ${t} serta memberikan cara ilmiah perbaikannya.`,
        exp: (t: string) => `Menguji kemampuan reflektif dan pemahaman kritis siswa terhadap kekeliruan materi ${t}.`
      },
      {
        q: (t: string) => `Buatlah rangkuman komprehensif atau peta konsep yang menghubungkan sub-materi dalam ${t} secara terstruktur!`,
        ans: (t: string) => `Kunci Jawaban: Rangkuman terintegrasi yang menunjukkan keterkaitan antar konsep dalam materi ${t}.`,
        exp: (t: string) => `Penilaian berfokus pada kelengkapan keterkaitan antar konsep ${t}.`
      },
      {
        q: (t: string) => `Bagaimana relevansi penerapan materi ${t} dalam kehidupan sehari-hari atau dunia industri/ilmu pengetahuan saat ini?`,
        ans: (t: string) => `Kunci Jawaban: Menyebutkan aplikasi praktis ${t} dalam kehidupan sehari-hari atau perkembangan teknologi.`,
        exp: (t: string) => `Mengukur wawasan kontekstual siswa terhadap pemanfaatan materi ${t}.`
      }
    ];
  ...
  /**
   * Gemini Proxy: AI Quiz Generator from text/topic/PDF content
   */
...
`;
      ...
      // Ensure questions is a valid array of items
      if (!Array.isArray(questions) || questions.length === 0) {
        questions = createDiverseFallbackQuestions(topicText, count, qTypeStr, gradeLevel);
      } else {
        // Sanitize and ensure proper options/types for all questions
  ...
  /**
   * Gemini Proxy: Semantic Essay Auto-Grader
   */
...
`;
  ...
  /**
   * Gemini Proxy: AI RPP & LKPD Generator (100 Essay Questions LKPD + Full RPP)
   */
...
3. Pengayaan & Remedial: Pengerjaan soal HOTS tingkat lanjut bagi siswa tuntas dan bimbingan tutor sebaya bagi siswa remedial.`;
      ...
      // Generate full 100 Uraian Questions for LKPD based on cleanTopic and curriculum
